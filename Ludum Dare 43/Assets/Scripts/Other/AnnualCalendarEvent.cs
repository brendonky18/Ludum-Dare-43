...
public class AnnualCalendarEvent : CalendarEvent {
    ...
    public AnnualCalendarEvent(int month, int day) : base(new Date(month, day, World.Instance.Date.YEAR /*This might be a problem*/), Time.Midnight, Time.EndOfDay) {
        //event automatically adds itself to the calendar for next year when this year's event ends
        this.onEnd += () => {
            World.Instance.AddEvent(new AnnualCalendarEvent(this));
        };
    }
